...
class StatusScreen extends Component
{
    ...
    public function render()
    {
        /* if the user is admin */
        if(Auth::user()->user_type==2) {
            $this->pending_orders = Invoice::where('status',1)->latest()->get();
            $this->processing_orders = Invoice::where('status',2)->latest()->get();
            $this->ready_orders = Invoice::where('status',3)->latest()->get();
        }   
